...
open class BubbleTabBar: UITabBar {
    ...
    override open func layoutSubviews() {
        ...
        roundCorners([.topLeft, .topRight], radius: 28)
    }
    private func configure() {
        self.layer.masksToBounds = true
        isTranslucent = false
        backgroundColor = .white
        barTintColor = UIColor(red: 0, green: 0.765, blue: 0.753, alpha: 1)
        tintColor = .white
        ...
        container.topAnchor.constraint(equalTo: topAnchor, constant: 13).isActive = true
        let bottomOffset: CGFloat
        if #available(iOS 11.0, *) {
            bottomOffset = safeAreaInsets.bottom == 0 ? 15 : safeAreaInsets.bottom
        } else {
            bottomOffset = 15
        }
