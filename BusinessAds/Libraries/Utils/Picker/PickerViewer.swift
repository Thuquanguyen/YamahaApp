...
class PickerViewer {
    ...
    static func showDatePickerScrollToSelect(date: Date, minDate: Date? = nil, maxDate: Date? = nil, completion: ((DateTimePickerDateResponse?) -> Void)?) {
        DateTimePicker.present(animated: false, prepare: { picker in
            picker.set(type: .date)
            picker.set(date: date, minDate: minDate, maxDate: maxDate)
            picker.didScrollToDate = { response in
                completion?(response)
            }
